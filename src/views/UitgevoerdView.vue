<template>
  <v-container>
                ...
                <v-row class="edit-goals-row">
                  ...
                  <v-col v-for="option in currentGoal?.options" :key="option.key" class="edit-goals-col">
                    ...
                    <template v-else-if="optionType(option.key) === 'image'">
                      <v-file-input @change="onImageChange($event, option)" prepend-icon="mdi-camera" accept="image/*" capture="camera"></v-file-input>
    ...
    <v-row v-if="!loggedInUser">
      <v-col>
        <p>Log in om de uitgevoerde inspecties te kunnen inzien.</p><br>
...
export default {
  ...
  computed: {
    ...
    filterInspections() {
      const store = useAppStore();
      // Filter inspecties op new_inspection: false
      const filteredInspections = store.inspections.filter(inspection => inspection.new_inspection === false);
  ...
  methods: {
    ...
    async editGoal(inspectionId, goal) {
      ...
      if (inspection) {
        ...
        // Zoek de option met de key "date"
        const dateOption = goal.options.find(option => option.key === 'date');
        if (dateOption) {
          // Als de option is gevonden, haal de waarde op
          const timestamp = dateOption.value;
          // Stel de currentGoalDate in op basis van de gevonden timestamp
          if (timestamp) {
            if (typeof timestamp.toDate === 'function') {
              this.currentGoalDate = timestamp.toDate();
            } else {
              this.currentGoalDate = new Date(timestamp);
            }
          } else {
    ...
    async saveInspection(inspectionId) {
      try {
        ...
        if (inspection) { 
          ...
          // Zet de new_inspection vlag op true als alle velden leeg zijn
          if (!hasData) {
            inspection.new_inspection = true;
          } else {
            inspection.new_inspection = false;
          }
          console.log('new_inspection set to:', inspection.new_inspection);
          ...
          setTimeout(async () => {
            // Reset de successMessage na 3 seconden
